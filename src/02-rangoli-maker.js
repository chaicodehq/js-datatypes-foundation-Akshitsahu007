...
export function repeatPattern(pattern, times) {
  // Your code 
  if (typeof (pattern) === "string" && pattern.trim().length !== 0 && Number.isInteger(times) && times > 0) {
...
export function extractRangoliCenter(design, start, end) {
  ...
  if (typeof (design) === "string" && design.trim().length !== 0 && typeof (start) === "number" && typeof (end) === "number") {
...
export function splitAndJoinRangoli(colorString, oldSep, newSep) {
  ...
  if (typeof (colorString) === "string" && String(oldSep).toString() && String(newSep).toString()) {
    return colorString.split(oldSep, -1).join(newSep);
  } else {
    return ""
  }
}
